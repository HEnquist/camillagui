                    ...
                    <div className="tabpanel" style={{ width: "700px" }}>
                        ...
                        {this.mixerNames().map((name) => (
                            <MixerView
                                ...
                                update={(update) =>
                                    this.updateMixer(name, update)
                                }
                                isFreeMixerName={this.isFreeMixerName}
                                rename={(newName) =>
                                    this.renameMixer(name, newName)
                                }
            ...
            title={
                <>
                    <ParsedInput
                        ...
                        asString={(name) => name}
                        parseValue={(name) =>
                            isValidMixerName(name) ? name : undefined
                        }
                        immediate={false}
                    />
                    <DeleteButton
        ...
        >
            ...
            <div className="vertically-spaced-content">
                <OptionalTextInput
                    ...
                    onChange={(desc) =>
                        update((mixer) => (mixer.description = desc))
                    }
                />
            </div>
        </Box>
                ...
                {Range(0, channels.out).map((dest) => {
                    ...
                    return (
                        <tr key={"row" + dest}>
                            ...
                            {Range(0, channels.in).map((src) => {
                                const [cell, map_idx, src_idx] = getSource(
                                    mixer.mapping,
                                    src,
                                    dest,
                                )
...
const makeDropdown = (
    ...
    const errorsForCell = errors.forSubpath(
        "mapping",
        map_idx,
        "sources",
        src_idx,
    ...
    return (
        <div
            ...
            style={{ borderColor: cssColorAt(cell, errorsForCell) }}
            title="channels"
        >
            <SourceCell
                source={cell}
                errors={errorsForCell}
                update={update}
                remove={remove}
                close={close}
            />
            ...
            <div className="vertically-spaced-content">
                ...
                >
                    <FloatInput
                        ...
                        onChange={(gain: number) =>
                            update((source) => (source.gain = gain))
                        }
                    />
                ...
                >
                    <EnumInput
                        ...
                        onChange={(scale) =>
                            update((source) => (source.scale = scale))
                        }
                    />
                </div>
                ...
                <div className="horizontally-spaced-content">
                    ...
                    <MdiButton
                        ...
                        onClick={() =>
                            update((source) => (source.mute = !source.mute))
                        }
                    />
                    <MdiButton
                        icon={mdiDelete}
