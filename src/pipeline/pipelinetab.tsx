                    ...
                    <DndContainer>
                        <div className="tabpanel" style={{ width: "700px" }}>
                            ...
                            <div className="pipeline-channel">
                                Capture: {this.state.capture_channels} channels
                                in
                            </div>
                            {pipeline?.map(
                            ...
                            <div className="horizontally-spaced-content">
                                ...
                                <PlotButton
                                    ...
                                    pipeline={true}
                                    onClick={() =>
                                        this.setState({ plotPipeline: true })
                                    }
                                />
                            </div>
                            <div className="pipeline-channel">
                                Playback: {config.devices.playback.channels}{" "}
                                channels out
                            </div>
                            <PipelinePopup
                                ...
                                config={config}
                                onClose={() =>
                                    this.setState({ plotPipeline: false })
                                }
                            />
...
function usePipelineStepDndSort(
    stepIndex: number,
            ...
            <Box
                title={
                    <>
                        <DragHandle
                            drag={dndProps.drag}
                            tooltip="Drag mixer to change order"
                        />
                        {typeSelect}
                        {channelInfo}
                        <OptionalBoolOption
            ...
            >
                <div className="vertically-spaced-content">
                    ...
                    <OptionalTextInput
                        ...
                        onChange={(desc) =>
                            update((step) => (step.description = desc))
                        }
                    />
                </div>
            </Box>
...
function FilterStepView(props: {
    ...
    const title = (
        <>
            <DragHandle
                drag={dndProps.drag}
                tooltip="Drag filter step to change order"
            />
            {typeSelect}
            <ChannelSelection
                ...
                setChannels={(channels) =>
                    update((step) => (step.channels = channels))
                }
                labels={props.labels}
                multiSelect={true}
            />
        ...
        <DndSortable {...dndProps}>
            <Box title={title}>
                <div className="vertically-spaced-content">
                    <ErrorMessage message={props.errors.messageFor("type")} />
                    <ErrorMessage
                        message={props.errors.messageFor("channel")}
                    />
                    <ErrorMessage message={props.errors.rootMessage()} />
                    <div className="vertically-spaced-content">
                    ...
                    <div className="horizontally-spaced-content">
                        {controls}
                        <AddButton
                            tooltip="Add a filter to the list"
                            onClick={addFilter}
                        />
                        <PlotButton
                            tooltip="Plot response of this step"
                            onClick={plot}
                        />
                    </div>
                    <OptionalTextInput
