                ...
                <div className="wide-tabpanel" style={{ width: "900px" }}>
                    ...
                    <FileTable
                        ...
                        setCurrentConfigFileName={
                            props.setCurrentConfigFileName
                        }
                        saveNotify={props.saveNotify}
                        canUpdateActiveConfig={
                            props.guiConfig.can_update_active_config
                        }
                    />
...
> {
    ...
    private upload(files: FileList) {
        doUpload(
            ...
            (message) =>
                this.showErrorMessage(EMPTY_FILENAME, "upload", message),
        )
    }

    private async loadConfig(name: string) {
        try {
            const jsonConfig = await loadConfigJson(name, (reason) =>
                this.showErrorMessage(name, "load", reason),
            )
    ...
    private async compareConfig(name: string) {
        try {
            const otherConfig = await loadConfigJson(name, (reason) =>
                this.showErrorMessage(name, "load", reason),
            )
    ...
    private async compareConfigFiles(name_left: string, name_right: string) {
        try {
            const leftConfig = await loadConfigJson(name_left, (reason) =>
                this.showErrorMessage(name_left, "load", reason),
            )
    ...
    private async plotConfig(name: string) {
        try {
            const config = await loadConfigJson(name, (reason) =>
                this.showErrorMessage(name, "load", reason),
            )
        ...
        if (this.type === "coeff") {
                ...
                cell: (
                    ...
                    <div style={{ display: "flex", flexDirection: "row" }}>
                        <SetActiveButton
                            ...
                            enabled={
                                this.props.canUpdateActiveConfig && row.valid
                            }
                            valid={row.valid}
                        />
                        <SaveButton
                ...
                cell: (
                    ...
                    <div>
                        <FileDownloadLink
                            ...
                            filename={row.name}
                            isCurrentConfig={
                                row.name === this.props.currentConfigFile
                            }
                        />
                        <FileStatusMessage
                            filename={row.name}
            ...
            columns.push({
                ...
                cell: (
                    ...
                    <div
                        data-tooltip-html={row.description}
                        data-tooltip-id="main-tooltip"
                    >
                        {row.title ? (
                            row.title
                        ) : row.description ? (
                            <i>{row.description.slice(0, 20) + "..."}</i>
                        ) : null}
                    </div>
                ),
            ...
            columns.push({
                ...
                cell: (
                    ...
                    <div
                        data-tooltip-html={fileStatusDesc(row.errors)}
                        data-tooltip-id="main-tooltip"
                    >
                        {row.valid === true ? "✔️" : "❌"}
                    </div>
...

                    {
                        ...
                        this.type === "config" && (
                            <>
                                ...
                                >
                                    <SaveButton
                                        disableReason={reasonToDisableSaveNewFileButton(
                                            newFileName,
                                            files,
                                        )}
...
function DownloadFilesAsZipButton(props: {
...
function DeleteFilesButton(props: {
    selectedFiles: string[]
    delete: () => {}
}) {
...
function SetActiveButton(props: {
    ...
    if (enabled === false) {
        if (valid) {
            ...
            tooltip =
                "Mark this config file as active.<br>Disabled since this config file is not valid."
        }
    } else {
        if (active) {
            ...
            tooltip =
                "Mark this config file as active, and load it into the GUI."
        }
    }
...
function SaveButton(props: {
    ...
    return (
        <MdiButton
            ...
            tooltip={
                disableReason ? disableReason : `Save from GUI to ${filename}`
            }
...
function CompareFilesButton(props: {
    ...
    const enabled =
        ...
        selectedFiles[0].valid &&
        selectedFiles[1].valid
    let tooltip
    if (enabled) {
    ...
    return (
        <MdiButton
            ...
            enabled={enabled}
            onClick={() =>
                compareConfigs(selectedFiles[0].name, selectedFiles[1].name)
            }
        />
    )
}
    ...
    return (
        <a
            ...
            data-tooltip-html={
                ...
                filename +
                (isCurrentConfig
                    ? "<br>This is the config file currently loaded in this Editor"
                    : "")
            }
            data-tooltip-id="main-tooltip"
            download={filename}
...
function reasonToDisableSaveNewFileButton(
    newFileName: string,
    files: FileInfo[],
): string | undefined {
