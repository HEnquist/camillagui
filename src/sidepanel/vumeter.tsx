...
export function VuMeterGroup(props: {
    ...
    const meters = (
        <canvas
            ...
            height={
                levels.length * meterHeightInPX +
                (levels.length + 1) * gapHeightInPX +
                2 * dbMarkerLabelHeight +
                "px"
            }
            ref={canvasRef}
        />
    ...
    useEffect(() => {
        ...
        Range(0, levels.length).forEach((index) => {
            ...
            drawLabel(
                context,
                css,
                getLabelForChannel(labels, index, true, false),
                index + 1,
                0,
            );
...
function drawDbMarkers(context: any, css: CSSStyleDeclaration, index: number) {
    ...
    dbMarkersAt.forEach((marker) => {
        ...
        context.fillRect(
            x,
            y + meterHeightInPX + gapHeightInPX,
            2,
            gapHeightInPX,
        );
    });
...
function drawDbMarkerLabels(
    ...
    dbMarkersAt.forEach((marker) => {
        ...
        context.fillRect(
            x,
            y + meterHeightInPX + dbMarkerHeight,
            2,
            dbMarkerHeight,
        );
        if (dbMarkersWithTextLabel.includes(marker)) {
            ...
            context.fillText(
                marker.toString(10),
                x + 1,
                y + gapHeightInPX + dbMarkerHeight + 1,
            );
        }
    });
}
...
function draw0DbMarker(context: any, css: CSSStyleDeclaration, index: number) {
    context.fillStyle = css.getPropertyValue("--text-color");
    context.fillRect(
        labelWidth + (meterWidth * levelAsPercent(0)) / 100 - 1,
        meterYOffset(index),
        2,
        meterHeightInPX,
    );
...
function drawLevelBars(
    ...
    context.fillStyle = css.getPropertyValue(
        clipped ? "--error-text-color" : "--success-text-color",
    );
    ...
    const peakX =
        ...
        Math.min(
            meterWidth - 2,
            Math.round((meterWidth * peakInPercent) / 100 - 1),
        );
