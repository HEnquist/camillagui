...
export class AuxFadersBox extends React.Component<Props, State> {
    private fadersPoller = new FadersPoller(
    ...
    private async setDspFaders(faders: Fader[], prevFaders: Fader[]) {
        for (const [idx, fader] of faders.entries()) {
            if (
                Math.round(fader.volume * 10) !==
                Math.round(prevFaders[idx].volume * 10)
            ) {
            ...
            return (
                <div style={{ display: "flex", flexDirection: "row" }}>
                    <input
                        ...
                        key={"vol" + index}
                        onChange={(e) =>
                            this.moveFader(index, e.target.valueAsNumber / 10.0)
                        }
                        data-tooltip-html={
                            "Aux" +
                            (index + 1) +
                            " " +
                            faders[index].volume.toFixed(1) +
                            "dB"
                        }
                ...
                title={
                    <>
                        <MdiButton
                            ...
                            tooltip={
                                visible
                                    ? "Hide Aux volume controls"
                                    : "Show Aux volume controls"
                            }
                            buttonSize="small"
                            highlighted={visible}
                            onClick={() => {
