...
class BarChart extends React.Component {
    ...
    appendBlock(labels, boxes, label, x, y) {
      var rect = {x: x-10, y: y-5, width: 20, height: 10, fill: "none", stroke: "black", "stroke-width": 1};
      var text = {x: x-10, y: y-10, text: label}
      labels.push(text);
      boxes.push(rect);
    }
    ...
    makeShapes(conf) {
      ...
      for (var n = 0; n < active_channels; n++) {
          const label = "ch "+n; 
          this.appendBlock(labels, boxes, label, 0, spacing_v*(-active_channels/2 + 0.5 + n))
          //channels.append([b])
      }
      var capturename;
  ...
  //
    ...
    //
      ...
      var text = d3.select(node).selectAll("text")
                          .data(labels)
                          .enter()
                          .append("text");
        ...
        <div>
          ...
          >
            <div className="modal">
              ...
              <div>
                <BarChart config={this.state.config} />
              </div>
            </div>
          </Popup>
        </div>
