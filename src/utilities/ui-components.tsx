...
export function PlotButton(props: {
    ...
    return (
        <MdiButton
            icon={
                props.pipeline ? mdiSitemapOutline : mdiChartBellCurveCumulative
            }
            tooltip={props.tooltip}
            onClick={props.onClick}
        ...
        <>
            ...
            >
                <IntInput
                    {...props}
                    className={
                        "setting-input" + (small ? " small-setting-input" : "")
                    }
...
export function OptionalIntInput(props: {
    ...
    return (
        <OptionalParsedInput
            {...props}
            immediate={true}
            asString={(int: number | null | undefined) =>
                int !== null && int !== undefined ? int.toString() : ""
            }
...
export function LabelListOption(props: {
    ...
    return (
        <div
            className="setting"
            data-tooltip-html="Optional labels for individual channels"
            data-tooltip-id="main-tooltip"
        >
...
export function ErrorMessage(props: { message?: string }) {
    return props.message ? (
        <div
            style={{ color: "var(--error-text-color)", whiteSpace: "pre-wrap" }}
        >
            {props.message}
        </div>
    ) : null;
...
export function EnumInput<OPTION extends string>(props: {
    ...
    return (
        <select
            ...
            data-tooltip-id="main-tooltip"
            onChange={(e) =>
                props.onChange(string_to_null(e.target.value) as OPTION)
            }
            style={props.style}
            className={
                value === "default"
        ...
        >
            {optionsArray.map((row) => (
                <option
                    key={row.label}
                    value={null_to_string(row.value)}
                    label={row.label}
                ></option>
            ))}
        </select>
...
export function OptionalBoolInput(props: {
    ...
    return (
        <select
            ...
            data-tooltip-id="main-tooltip"
            onChange={(e) =>
                props.onChange(string_to_optional_bool(e.target.value))
            }
            style={props.style}
            className={
                props.value === null
                    ...
                    {Range(0, rows).map((row) => (
                        <tr>
                            {Range(
                                ...
                                Math.min(
                                    rowSize,
                                    maxChannelCount - rowSize * row,
                                ),
                            ).map((col) => (
                                <td>
                                    <ChannelButton
                                        key={rowSize * row + col}
                                        channel={getLabelForChannel(
                                            labels,
                                            rowSize * row + col,
                                            true,
                                            false,
                                        )}
                                        tooltip={getLabelForChannel(
                                            labels,
                                            rowSize * row + col,
                                            false,
                                        )}
    ...
    return (
        <Button
            ...
            style={{
                height: "28px",
                marginRight: "5px",
                backgroundColor: erroneousChannel
                    ? "var(--error-field-background-color)"
                    : undefined,
            }}
        />
    );
