...
function asFormattedLines(object: any): string[] {
  return Object.entries(object).flatMap(([key, value]) => {
    if (isArray(value) && value.every((item) => typeof item === "number")) return [key + "=" + value.join(",")]
    else if (isComplexObject(value)) return [key].concat(asFormattedLines(value).map((v) => "  " + v))
