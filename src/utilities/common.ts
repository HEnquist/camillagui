...
export function modifiedCopyOf<T>(
    object: T,
    modification: (copy: T) => void,
): T {
...
function removeEmptyProperties(object: any): any {
    Object.entries(object).forEach(([key, value]: [string, any]) => {
        if (
            ...
            delete object[key];
        else if (isComplexObject(value)) removeEmptyProperties(value);
    });
}

export function isComplexObject(item: any): boolean {
    return (
        item !== null && item !== undefined && (isArray(item) || isObject(item))
    );
}
...
function asFormattedLines(object: any): string[] {
    return Object.entries(object).flatMap(([key, value]) => {
        ...
        else if (isArray(object)) return [(value as any).toString()];
...
export function setNumberValue(object: any, path: string[], value: number) {
    ...
    for (const property of path.slice(0, path.length - 1)) {
        ...
        if (subObject === undefined) return undefined;
    }
    if (
        path[path.length - 1] in subObject &&
        typeof subObject[path[path.length - 1]] === "number"
    ) {
...
export function setBoolValue(object: any, path: string[], value: boolean) {
    ...
    if (
        path[path.length - 1] in subObject &&
        typeof subObject[path[path.length - 1]] === "boolean"
    ) {
        subObject[path[path.length - 1]] = value;
    }
}
