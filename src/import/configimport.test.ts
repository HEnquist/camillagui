...
describe("toggleTopLevelElement", () => {
    test("import primitive", () => {
        const toImport = new Import({ a: 0 }).toggleTopLevelElement(
            "a",
            "import",
        ).configToImport;
    ...
    test("import primitive into non-empty object", () => {
        const toImport = new Import({ a: 0 }, { b: 1 }).toggleTopLevelElement(
            "a",
            "import",
        ).configToImport;
    ...
    test("import object", () => {
        const toImport = new Import({ a: { b: {} } }).toggleTopLevelElement(
            "a",
            "import",
        ).configToImport;
    ...
    test("imported objects are cloned deep", () => {
        const original = { a: { b: {} } };
        const toImport = new Import(original).toggleTopLevelElement(
            "a",
            "import",
        ).configToImport;
    ...
    test("import array", () => {
        const toImport = new Import({ a: [{}] }).toggleTopLevelElement(
            "a",
            "import",
        ).configToImport;
...
describe("isTopLevelElementImported", () => {
    test("is imported", () => {
        expect(
            new Import({ a: 0 }, { a: 0 }).isTopLevelElementImported("a"),
        ).toBe(true);
    });
    ...
    test("differing element is partially imported", () => {
        expect(
            new Import({ a: 0 }, { a: 1 }).isTopLevelElementImported("a"),
        ).toBe("partially");
    });
});
...
describe("toggleSecondLevelElement", () => {
    ...
    test("import object and create missing parent", () => {
        const toImport = new Import({ a: { b: {} } }).toggleSecondLevelElement(
            "a",
            "b",
            "import",
        ).configToImport;
    ...
    test("imported objects are cloned deep", () => {
        const original = { a: { b: {} } };
        const toImport = new Import(original).toggleSecondLevelElement(
            "a",
            "b",
            "import",
        ).configToImport;
    ...
    test("import array with duplicates - first duplicate is imported", () => {
        const toImport = new Import({ a: ["b", "b"] }).toggleSecondLevelElement(
            "a",
            "0",
            "import",
        ).configToImport;
    ...
    test("remove array element", () => {
        const toImport = new Import({}, { a: [0, 1] }).toggleSecondLevelElement(
            "a",
            "1",
            "remove",
        ).configToImport;
...
describe("isSecondLevelElementImported", () => {
    test("is imported", () => {
        expect(
            new Import(
                { a: { b: 0 } },
                { a: { b: 0 } },
            ).isSecondLevelElementImported("a", "b"),
        ).toBe(true);
    });

    test("with missing parent element is not imported", () => {
        expect(
            new Import({ a: { b: 0 } }, {}).isSecondLevelElementImported(
                "a",
                "b",
            ),
        ).toBe(false);
    ...
    test("missing element is not imported", () => {
        expect(
            new Import({ a: { b: 0 } }, { a: {} }).isSecondLevelElementImported(
                "a",
                "b",
            ),
        ).toBe(false);
    ...
    test("is imported in array", () => {
        expect(
            new Import({ a: ["b"] }, { a: ["b"] }).isSecondLevelElementImported(
                "a",
                "0",
            ),
        ).toBe(true);
    ...
    test("only second element is imported in array", () => {
        const i = new Import({ a: ["b", "c"] }).toggleSecondLevelElement(
            "a",
            "1",
            "import",
        );
    ...
    test("duplicates in array are handled properly - first duplicate is imported", () => {
        const i = new Import({ a: ["b", "b"] }).toggleSecondLevelElement(
            "a",
            "0",
            "import",
        );
...
describe("used items from pipeline are automatically imported", () => {
    test("importing a pipeline step automatically imports all used items", () => {
        ...
        const i = new Import(config).toggleTopLevelElement(
            "pipeline",
            "import",
        );
    ...
    test("importing only one pipeline step imports only one item", () => {
        ...
        const i = new Import(config).toggleSecondLevelElement(
            "pipeline",
            "0",
            "import",
        );
    ...
    test("only items not used in pipeline steps are editable", () => {
        ...
        expect(i.isSecondLevelElementEditable("filters", "unusedFilter")).toBe(
            true,
        );
        expect(i.isSecondLevelElementEditable("mixers", "unusedMixer")).toBe(
            true,
        );
        expect(
            i.isSecondLevelElementEditable("processors", "unusedProcessor"),
        ).toBe(true);
        ...
        expect(i.isSecondLevelElementEditable("processors", "processor")).toBe(
            false,
        );
    });
});
