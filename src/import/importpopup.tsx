...
> {
    ...
    private setImportConfig(name: string, config: ImportedConfig) {
        this.setState({
            importDoneFromFile: undefined,
            importConfig: { name, config },
        });
    }
    ...
    render() {
        if (!("close" in this.props)) return null;
                    ...
                    {importConfig ? (
                        <ConfigItemSelection
                            ...
                            import={(configToImport) => {
                                ...
                                this.setState({
                                    importDoneFromFile: importConfig?.name,
                                    importConfig: undefined,
                                });
                            }}
                            cancel={() => this.close()}
...
function FileList(props: {
    ...
    function loadLocalCdspConfig(files: FileList): void {
        const file = files[0];
        importedYamlConfigAsJson(files).then((config) =>
            setImportConfig(file.name, config),
        );
    }
    function loadLocalConvolverConfig(files: FileList) {
        const file = files[0];
        importedConvolverConfigAsJson(files).then((config) =>
            setImportConfig(file.name, config),
        );
    }
    function loadLocalEqAPOConfig(files: FileList) {
        const file = files[0];
        importedEqAPOConfigAsJson(files, 2).then((config) =>
            setImportConfig(file.name, config),
        );
    }
    function loadJsonConfigWithName(name: string): void {
        loadMigratedConfigJson(name).then((config) =>
        ...
        <div className="tabpanel">
            {importDoneFromFile ? (
                <div style={bottomMargin}>
                    Import from {importDoneFromFile} was successful.
                    <br />
                    <br />
                ...
                <div style={bottomMargin}>
                    Select from which file to import.
                </div>
            )}
            <Box title="Upload new config file">
                ...
                >
                    <UploadButton
                        text="CamillaDSP Config"
                        tooltip="Import selected parts from a<br>CamillaDSP config file.<br>Configs for CamillaDSP<br>v1 and v2 are supported."
                        upload={loadLocalCdspConfig}
                    />
                    <UploadButton
                        text="Convolver Config"
                        tooltip="Translate a 'Convolver' config file<br>and import selected parts."
                        upload={loadLocalConvolverConfig}
                    />
                    <UploadButton
                        text="EqAPO Config"
                        tooltip="Translate an 'Equalizer APO' config file<br>and import selected parts."
                        upload={loadLocalEqAPOConfig}
                    />
                </div>
            </Box>
                ...
                {fileList.map((file) => (
                    <div key={file}>
                        <Button
                            ...
                            text={file}
                            onClick={() => loadJsonConfigWithName(file)}
                        />
                        <br />
                    </div>
                ))}
...
function ConfigItemSelection(props: {
    ...
    const [configImport, setConfigImport] = useState<Import>(
        new Import(config),
    );
    const topLevelConfigElements = Object.keys(
        withoutEmptyProperties(config),
            ...
            <div style={bottomMargin}>
                {
                    <>
                        <CheckBox
                            ...
                            onChange={(checked) =>
                                setConfigImport(
                                    new Import(config, checked ? config : {}),
                                )
                            }
                        />
                        <br />
                ...
                {topLevelConfigElements.map((parentKey) => {
                    ...
                    return (
                        <div key={parentKey}>
                            <CheckBox
                                text={parentKey}
                                checked={configImport.isTopLevelElementImported(
                                    parentKey,
                                )}
                                onChange={(checked) =>
                                    setConfigImport((prev) =>
                                        prev.toggleTopLevelElement(
                                            parentKey,
                                            checked ? "import" : "remove",
                                        ),
                                    )
                                }
                                    ...
                                    .map(([key, subValue]) => {
                                        return (
                                            ...
                                            >
                                                <CheckBox
                                                    text={key}
                                                    editable={configImport.isSecondLevelElementEditable(
                                                        parentKey,
                                                        key,
                                                    )}
                                                    checked={configImport.isSecondLevelElementImported(
                                                        parentKey,
                                                        key,
                                                    )}
                                                    onChange={(checked) =>
...
function valueAppended(value: any, tooltipId?: string): ReactNode {
    if (!isComplexObject(value))
        return (
            <span style={{ color: "var(--disabled-text-color)" }}>
                : {value.toString()}
            </span>
        );
...
function collisionWarning(
    ...
    if (
        ["filters", "mixers", "processors"].includes(parentKey) &&
        parentKey in config &&
