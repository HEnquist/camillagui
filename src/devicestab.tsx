                ...
                <div className="tabpanel">
                    ...
                    <Samplerate
                        hide_capture_samplerate={
                            guiConfig.hide_capture_samplerate
                        }
                        devices={devices}
                        errors={errors}
                    ...
                    <CaptureOptions
                        ...
                        supported_capture_types={
                            availableBackends[1] as CaptureType[]
                        }
                    ...
                    <PlaybackOptions
                        ...
                        supported_playback_types={
                            availableBackends[0] as PlaybackType[]
                        }
        ...
        >
            ...
            {isNonDefaultSamplerate(samplerate) && (
                <OptionalIntInput
                    ...
                    className="setting-input"
                    style={{ width: "60%" }}
                    onChange={(samplerate: number | null) =>
                        props.onChange(samplerate)
                    }
                />
            )}
            <ErrorMessage message={props.error} />
    ...
    return (
        <Box title="Buffers">
            <IntOption
                ...
                onChange={(chunksize) =>
                    props.onChange((devices) => (devices.chunksize = chunksize))
                }
            />
            <OptionalIntOption
                ...
                onChange={(queuelimit) =>
                    props.onChange(
                        (devices) => (devices.queuelimit = queuelimit),
                    )
                }
            />
        </Box>
    ...
    return (
        <Box title="Silence">
            <OptionalFloatOption
                ...
                onChange={(silenceThreshold) =>
                    props.onChange(
                        (devices) =>
                            (devices.silence_threshold = silenceThreshold),
                    )
                }
            />
            <OptionalFloatOption
                ...
                onChange={(silenceTimeout) =>
                    props.onChange(
                        (devices) => (devices.silence_timeout = silenceTimeout),
                    )
                }
            />
        </Box>
    ...
    return (
        <Box title="Rate adjust">
            <OptionalBoolOption
                ...
                onChange={(enableRateAdjust) =>
                    props.onChange(
                        (devices) =>
                            (devices.enable_rate_adjust = enableRateAdjust),
                    )
                }
            />
            <OptionalIntOption
                ...
                onChange={(adjustPeriod) =>
                    props.onChange(
                        (devices) => (devices.adjust_period = adjustPeriod),
                    )
                }
            />
            <OptionalIntOption
                ...
                onChange={(targetLevel) =>
                    props.onChange(
                        (devices) => (devices.target_level = targetLevel),
                    )
                }
            />
        </Box>
        ...
        <Box title="Resampling">
            <EnumOption
                ...
                onChange={(resamplerType) =>
                    props.onChange(
                        (devices) =>
                            (devices.resampler =
                                changeResamplerType(resamplerType)),
                    )
                }
            />
            {devices.resampler && devices.resampler.type === "AsyncSinc" && (
                <EnumOption
                    // @ts-ignore
                    value={
                        devices.resampler.hasOwnProperty("profile")
                            ? devices.resampler.profile
                            : "Free"
                    }
                    ...
                    onChange={(profile) =>
                        props.onChange(
                            (devices) =>
                                (devices.resampler =
                                    changeResamplerProfile(profile)),
                        )
                    }
                />
            )}
            {devices.resampler &&
                devices.resampler.type === "AsyncSinc" &&
                !devices.resampler.hasOwnProperty("profile") && (
                    <>
                        <EnumOption
                            // @ts-ignore
                            ...
                            onChange={(interp) =>
                                props.onChange(
                                    (devices) =>
                                        (devices.resampler.interpolation =
                                            interp),
                                )
                            }
                        />
                        ...
                        <EnumOption
                            ...
                            onChange={(window) =>
                                props.onChange(
                                    (devices) =>
                                        (devices.resampler.window = window),
                                )
                            }
                        />
                    </>
    ...
    return (
        <Box title="Capture rate monitoring">
            ...
            <OptionalBoolOption
                ...
                onChange={(stopOnRateChange) =>
                    props.onChange(
                        (devices) =>
                            (devices.stop_on_rate_change = stopOnRateChange),
                    )
                }
            />
        </Box>
    ...
    return (
        <Box title="Multithreaded processing">
            <OptionalBoolOption
                ...
                onChange={(multithreaded) =>
                    props.onChange(
                        (devices) => (devices.multithreaded = multithreaded),
                    )
                }
            />
            <OptionalIntOption
                ...
                onChange={(workerThreads) =>
                    props.onChange(
                        (devices) => (devices.worker_threads = workerThreads),
                    )
                }
            />
        </Box>
    ...
    return (
        <Box title="Volume control settings">
            <OptionalFloatOption
                ...
                onChange={(volumeRampTime) =>
                    props.onChange(
                        (devices) =>
                            (devices.volume_ramp_time = volumeRampTime),
                    )
                }
            />
            <OptionalFloatOption
                ...
                onChange={(volumeLimit) =>
                    props.onChange(
                        (devices) => (devices.volume_limit = volumeLimit),
                    )
                }
            />
        </Box>
        ...
        return (
            <div>
                {Range(0, channels).map((row) => (
                    <OptionalTextOption
                        value={
                            capture.labels && capture.labels.length > row
                                ? capture.labels[row]
                                : null
                        }
                        ...
                        onChange={(new_label) =>
                            updateChannelLabel(row, new_label)
                        }
                    />
                ))}
            </div>
        ...
        <Box title="Capture device">
            ...
            <EnumOption
                ...
                onChange={(captureType) =>
                    onChange(
                        (devices) => (devices.capture = defaults[captureType]),
                    )
                }
            />
            {capture.type !== "WavFile" && (
                <IntOption
                    ...
                    onChange={(channels) =>
                        onChange(
                            (devices) => (devices.capture.channels = channels),
                        )
                    }
                />
            )}
                        ...
                        onChange={(link_volume_control) =>
                            onChange(
                                (
                                    ...
                                    (devices.capture.link_volume_control =
                                        link_volume_control),
                            )
                        }
                    />
                        ...
                        onChange={(link_mute_control) =>
                            onChange(
                                (
                                    ...
                                    (devices.capture.link_mute_control =
                                        link_mute_control),
                            )
                        }
                    />
                        ...
                        onChange={(node_description) =>
                            onChange(
                                (
                                    ...
                                    (devices.capture.node_description =
                                        node_description),
                            )
                        }
                    />
                        ...
                        onChange={(node_group_name) =>
                            onChange(
                                (
                                    ...
                                    (devices.capture.node_group_name =
                                        node_group_name),
                            )
                        }
                    />
                        ...
                        onChange={(autoconnect_to) =>
                            onChange(
                                (
                                    ...
                                    (devices.capture.autoconnect_to =
                                        autoconnect_to),
                            )
                        }
                    />
        ...
        <Box title="Playback device">
            ...
            <EnumOption
                ...
                onChange={(playbackType) =>
                    props.onChange(
                        (devices) =>
                            (devices.playback = defaults[playbackType]),
                    )
                }
            />
            <IntOption
                ...
                onChange={(channels) =>
                    onChange(
                        (devices) => (devices.playback.channels = channels),
                    )
                }
            />
            {(playback.type === "File" ||
                        ...
                        onChange={(node_description) =>
                            onChange(
                                (
                                    ...
                                    (devices.playback.node_description =
                                        node_description),
                            )
                        }
                    />
                        ...
                        onChange={(node_group_name) =>
                            onChange(
                                (
                                    ...
                                    (devices.playback.node_group_name =
                                        node_group_name),
                            )
                        }
                    />
                        ...
                        onChange={(autoconnect_to) =>
                            onChange(
                                (
                                    ...
                                    (devices.playback.autoconnect_to =
                                        autoconnect_to),
                            )
                        }
                    />
