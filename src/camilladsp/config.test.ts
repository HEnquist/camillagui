...
test("renameFilter", () => {
    ...
    expect(config.pipeline[0].names).toEqual([
        "renamed",
        "filter1",
        "renamed",
        "filter2",
...
test("renameFilter throws on name collision", () => {
    ...
    expect(() => renameFilter(config, "to be renamed", "collision")).toThrow(
        "Filter 'collision' already exists",
    )
})
...
test("renameMixer throws on name collision", () => {
    ...
    expect(() => renameMixer(config, "to be renamed", "collision")).toThrow(
        "Mixer 'collision' already exists",
    )
})
